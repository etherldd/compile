...
typedef struct InterCode {
    ...
    union {
        ...
        struct { Operand result, op1, op2; }    op3;      //ADD, SUB, MUL, DIV
        struct { Operand place; }               goto_st;  //GOTO
        struct { Operand label_name; }          label;    //LABEL
        struct { Operand func_name; }           func;     //FUNC
        struct { Operand op1, op2; 
                 char*   op;
                 Operand desti;}                if_st;    //IF
        struct { Operand val; }                 return_st;//RETURN
        struct { Operand opr; Operand size; }   dec;      //DEC
        struct { Operand val; }                 arg;      //ARG
        struct { Operand ret, func; }           call;     //CALL
        struct { Operand val; }                 param;    //PARAM
        struct { Operand val; }                 read;     //READ
        struct { Operand val; }                 write;    //WRITE
    } u;
} IR;
